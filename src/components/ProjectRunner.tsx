...
export default function ProjectRunner() {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-full min-h-[600px] lg:min-h-0 overflow-hidden relative">
      
      {/* Mobile Toggle Bar */}
      <div className="lg:hidden flex bg-background border-b border-border p-2 shrink-0 shadow-sm z-10 sticky top-0">
        <div className="flex w-full bg-black/5 dark:bg-white/5 rounded-lg p-1">
          <button
            onClick={() => setMobileTab('theory')}
            className={`flex-1 py-2 px-4 text-sm font-medium rounded-md transition-all ${mobileTab === 'theory' ? 'bg-background shadow text-primary' : 'text-text-muted hover:text-foreground'}`}
          >
            Instructions
          </button>
          <button
            onClick={() => setMobileTab('code')}
            className={`flex-1 py-2 px-4 text-sm font-medium rounded-md transition-all ${mobileTab === 'code' ? 'bg-background shadow text-primary' : 'text-text-muted hover:text-foreground'}`}
          >
            Code Editor
          </button>
        </div>
      ...
      <div className={`w-full lg:w-2/5 lg:border-r border-border bg-background p-6 flex-col overflow-y-auto ${mobileTab === 'code' ? 'hidden lg:flex' : 'flex'} flex-1 lg:flex-none h-full`}>
        <div className="flex items-center text-xs font-semibold text-primary mb-4 uppercase tracking-wider shrink-0">
