...
const hoverDict: Record<string, { label: string; desc: string }> = {
  ...
  'try':      { label: 'try / catch', desc: 'Attempts to execute a block of code. If an error is thrown (or a Promise rejected), execution jumps to the catch block.' },
  '__dirname': { label: '__dirname', desc: 'A global variable holding the absolute path of the directory containing the currently executing file.' },
  'fs':    { label: 'fs (File System)', desc: 'The native Node.js module used to interact with the file system.' },
  'path':  { label: 'path', desc: 'The native Node.js module used for handling and transforming file paths.' },
  ...
  '.join':   { label: 'path.join(...paths)',    desc: 'Safely joins all given path segments together using the platform-specific separator (e.g. \\ on Windows, / on macOS).' },
  '.readFileSync': { label: 'fs.readFileSync(path)', desc: 'Reads a file synchronously. Warning: This completely blocks the Event Loop until the file is fully loaded into memory!' },
  '.readFile': { label: 'fs.readFile(path)',    desc: 'Reads a file asynchronously. The Event Loop continues spinning while Libuv reads the file in the background.' },
  '.watch':  { label: 'fs.watch(filename)',     desc: 'Watches for changes on a file or directory by hooking natively into OS file events (like inotify). Extremely CPU efficient.' },
  '.createReadStream': { label: 'fs.createReadStream()', desc: 'Opens a Readable Stream to a file. Instead of loading the whole file into RAM, it reads it in tiny manageable chunks.' },
  '.readdir': { label: 'fs.readdir(path)',      desc: 'Reads the contents of a directory (returns an array of names).' },
  '.isDirectory': { label: 'item.isDirectory()', desc: 'Returns true if the current fs.Dirent object represents a folder.' },
